  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-serif text-[#1A1A1A] mb-2">Users</h1>
      ...
      {loading ? (
        ...
        <div className="bg-white p-12 rounded-lg text-center border border-[#E0E0E0]">
            <p className="text-[#888]">No users found or endpoint not available.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {users.map((user) => (
                <div key={user.id} className="bg-white p-6 rounded-lg shadow-sm border border-[#E0E0E0]">
                    <div className="flex items-center gap-4 mb-4">
                        <div className="w-12 h-12 bg-[#F5F5F5] rounded-full flex items-center justify-center text-xl font-serif text-[#1A1A1A]">
                            {user.name.charAt(0)}
                        </div>
                        <div>
                    ...
                    <div className="space-y-3 text-sm text-[#555]">
                        ...
                        <div className="flex items-center gap-3">
                            <Phone size={16} className="text-[#AAA]" />
                            <span>{user.phone}</span>
                        </div>
                        {user.createdAt && (
                             <div className="flex items-center gap-3">
                                <Calendar size={16} className="text-[#AAA]" />
