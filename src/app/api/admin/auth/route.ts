...
export async function POST(request: NextRequest) {
  try {
    ...
    if (secretKey !== process.env.ADMIN_SECRET_KEY) return errorResponse('Invalid credentials', 401)

    const normalizedEmail = email.toLowerCase().trim()
    const user = await db.query.users.findFirst({ where: eq(users.email, normalizedEmail) })
    if (!user || user.role !== 'ADMIN') return errorResponse('Admin account not found', 404)
