import api, { Product, User, Order, Coupon, Outfit, OutfitDetail, HeroContent, Reel, Review } from "./api";
...
export const adminOutfitsApi = {
  getAll: (params?: { page?: number; limit?: number; type?: string }) => {
    ...
    const query = searchParams.toString();
    return api<{ outfits: Outfit[]; totalCount: number }>(`/api/admin/outfits${query ? `?${query}` : ""}`);
  },
  getById: (id: string) => api<OutfitDetail>(`/api/admin/outfits/${id}`),
  create: (data: Partial<Outfit>) => api<{ outfit: Outfit; message: string }>("/api/admin/outfits", { method: "POST", body: data }),
  delete: (id: string) => api<{ message: string }>(`/api/admin/outfits/${id}`, { method: "DELETE" }),
};
...
export const adminReelsApi = {
  getAll: () => api<{ reels: Reel[] }>("/api/admin/reels"),
  create: (data: Partial<Reel>) => api<{ reel: Reel; message: string }>("/api/admin/reels", { method: "POST", body: data }),
  update: (id: string, data: Partial<Reel>) => api<{ reel: Reel; message: string }>(`/api/admin/reels/${id}`, { method: "PATCH", body: data }),
  delete: (id: string) => api<{ message: string }>(`/api/admin/reels/${id}`, { method: "DELETE" }),
};
